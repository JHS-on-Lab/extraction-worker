...
| 인자 | 설명 | 값 범위 | 기본값 |
|---|---|---|---|
| `--source` | 처리할 소스 지정, 콤마로 복수 지정 가능 (예: `NAVER_NEWS,DAUM_NEWS`) | `NAVER_NEWS` \| `DAUM_NEWS` \| `GOOGLE_NEWS` \| `BAIDU_NEWS` \| `NAVER_STOCK` \| `DUCKDUCKGO_NEWS` \| `BAOMOI_NEWS` \| `TINHTE_FORUM` \| `all` | `all` |
